...
export function ExplorationCard({ exploration }: Props) {
  return (
    <Card className="overflow-hidden border-white/10 bg-white/8 text-white shadow-xl backdrop-blur">
      <div
      ...
      <CardContent className="p-4">
        ...
        <p className="mt-3 line-clamp-2 text-sm leading-6 text-white/70">{exploration.summaryMemo}</p>

        <div className="mt-4 space-y-2">
          {exploration.places.slice(0, 2).map((place) => (
            <div key={place.id} className="rounded-2xl bg-black/25 p-3">
