...
`http://localhost:3000` 에서 확인할 수 있습니다 (백엔드 `roamit-api`는 3001 포트를 사용합니다).

API를 호출하려면 별도로 `roamit-api`를 실행하고, 그 저장소의 `CORS_ORIGIN` 환경변수에 `http://localhost:3000`이 등록되어 있어야 합니다 (기본값에 포함되어 있음).
